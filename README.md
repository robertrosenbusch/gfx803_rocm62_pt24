...
3. build your Docker image via `docker build . -t 'rocm62_pt24:latest'`
3.1 --> if you trust my compiled WHL-Files into this git-repo, just comment out from Line 76 to Line 116 and import the whl-Files into the container
4. start the container via:
5. `docker run -it --device=/dev/kfd --device=/dev/dri --group-add=video --ipc=host --cap-add=SYS_PTRACE --security-opt seccomp=unconfined  -p 8188:8188 rocm62_pt24:latest`
